...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda_', type=float, default=1)                      # lambda hyparams              
    parser.add_argument('--p', type=int, default=5)                              # p hyparams
    ...
    parser.add_argument('--energy', action='store_true', default=True)
    parser.add_argument('--energy_loss', action='store_true', default=True)
    ...
    parser.add_argument('--top_method', type=str, default='query', choices=['query', 'proto', 'all'])
    ...
    parser.add_argument('--cross', type=str, default=None, choices=['MiniImageNet', 'TieredImageNet', 'CIFAR-FS',
                                                                    'FC100', 'cub'])
    # method
    parser.add_argument('--method', type=str, default="GEL", choices=["SnaTCHer", "SFCM"])
    ...
    if args.method == "SFCM":
        from model.trainer.fsor_trainer_SFCM import FSORTrainerOurs
        trainer = FSORTrainerOurs(args)
        if not args.test:
            trainer.train()
        print('evaluate_test')

...
def CML(instance_embs, gt_label, args):
    scores = instance_embs.float() @ instance_embs.float().T    # cos matrix
    w = (gt_label.float().unsqueeze(0).T == gt_label.float().unsqueeze(0)).float() # ground truth matrix w(i,j) = 1 if yi = yj
    ...
    gamma = 0.1
    lam = 0.4
    loss_context, reg_loss = contrastive_jaccard(scores, w, reg_sim, eps, args) # context simiraty
    loss_contrast = margin_contrastive(scores, w, pos_margins = pos_margins, neg_margins = neg_margins)
    loss_ctx =  lam * loss_context + gamma * reg_loss + (1. - lam - gamma) * loss_contrast
...
def get_jaccard_similarity(w, scores, k, eps):
    ...
    vk, ik = (-D).topk(k) # get k closest neighbors(values indices)
    ...
    Rk_over_2_mask = Nk_over_2_mask * Nk_over_2_mask.T  # 普通乘法(Nk_over_2_mask必须为方阵)
...
def contrastive_jaccard(scores, w, reg_sim, eps, args):

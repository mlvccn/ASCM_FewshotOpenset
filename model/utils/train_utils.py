...
def calc_auroc(known_scores, unknown_scores):
    y_true = np.array([0] * len(known_scores) + [1] * len(unknown_scores))  # know label 0  unknow lable 1 unknow score应当更大
    y_score = np.concatenate([known_scores, unknown_scores])
    # y_pred = np.where(y_score >= np.sort(y_score)[75], 1, 0)
    y_pred = np.where(y_score >= np.sort(y_score)[-1], 1, 0) # error
    fpr, tpr, thresholds = roc_curve(y_true, y_score)   # 将y_score中的值依次作为阈值 label越大 score值越大 roc值越高

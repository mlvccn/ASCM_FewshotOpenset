...
from model.models.base_ours import Base
from pytorch_metric_learning import losses

from model.modules.enhance import ASF


class SFCM(Base):
    def __init__(self, args):
        super(SFCM, self).__init__(args)
        self.args = args
        ...
        # self.metricLoss = losses.CircleLoss()
        self.ASFFM = ASF()

    def forward(self, x):
        # feature extraction
        instance_feats = self.encoder(x)    # resnet12 无avg_pooling
        ...
        instance_embs = self.classify(x, is_emb=True)   # embedding vector

        self.instance_embs = instance_embs  # 保存嵌入向量  
        
        if self.args.pixel_conv:
            instance_feats = self.conv(instance_feats)  # channel 减半 640 -> 320
    ...
    def _forward(self, instance_feats, instance_embs, support_idx, query_idx):
        self.emb_dim = instance_embs.size(-1)       # 640
        self.feat_dim = instance_feats.shape[1:]    # 640, 5, 5

        proto, kquery, uquery = self.embedding_process(instance_embs, support_idx, query_idx)       # embedding vector
        f_proto, f_kquery, f_uquery = self.feature_process(instance_feats, support_idx, query_idx)  # feature map


        d_proto = self.ASFFM(f_proto, proto)
        
        proto = proto + 0.1 * d_proto

        self.num_batch = proto.shape[0]
        self.num_proto = proto.shape[1]

        return (self.get_logits(proto, kquery, uquery), # know query and unkonw query的距离矩阵 class-wise metirc
                self.get_feat_logits(f_proto, f_kquery, f_uquery, distance=self.args.distance)) # fine-grained metric
    ...
    def feature_process(self, instance_feats, support_idx, query_idx):
        ...
        kquery = query[:, :, :closed_way].contiguous()      # (NTask, shot, way, d)
    ...
    def get_feat_logits(self, proto, kquery, uquery, distance="euclidean"):
        if distance == "euclidean":
        ...
        elif distance == "pixel_sim":   # proto (B, way, C, H, W) (1, 5, 320, 5, 5) query (B, way, shot, C, H, W)
            kquery = kquery.view(-1, self.feat_dim[0], self.feat_dim[1] * self.feat_dim[2]  # (N_q, C, H * W)
                                 ).unsqueeze(1).permute(0, 1, 3, 2).unsqueeze(-1)   #(N, 1, H*W, C, 1) (25, 1, 25, 320, 1)
            ...
            proto = proto.squeeze().view(-1, self.feat_dim[0], self.feat_dim[1] * self.feat_dim[2]
                                         ).unsqueeze(0).unsqueeze(2)    # (1, 5, 1, 320, 25)

            klogits = torch.nn.CosineSimilarity(dim=3)(kquery, proto)   # 25个320 维向量余弦相似度 两两做余弦相似度
            ulogits = torch.nn.CosineSimilarity(dim=3)(uquery, proto)   # 25, 5, 25, 25
            if self.args.top_method == 'query':     # top_method
                klogits = klogits.topk(self.args.top_k, dim=3).values.sum(dim=[2, 3]) / self.args.top_k # 此处的topk指的是前k个密集向量

...
class Base(nn.Module):
    ...
    def split_instances(self):
        args = self.args
        if self.training:
            return (torch.Tensor(np.arange(args.way * args.shot)).long().view(1, args.shot, args.way), # (bsz, shot, way)
                    torch.Tensor(np.arange(args.way * args.shot,
                                           args.way * (args.shot + args.query))).long().view(1, args.query, args.way))  # (bsz, query, way)
        else:
            return (
    ...
    def embedding_process(self, instance_embs, support_idx, query_idx):
        ...
        self.proto = proto          # 支持集原型
        self.kquery = kquery        # 查询集中的已知类样本
        self.uquery = uquery        # 查询集中的未知类样本
    ...
    def get_logits(self, proto, kquery, uquery):
        ...
        klogits = - torch.sum((kproto - kquery) ** 2, 2) / self.args.temperature    # negative euclidean distance

...
from model.models.SFCM import SFCM
from model.trainer.helpers import get_dataloader, prepare_model, prepare_optimizer
from model.utils.train_utils import Averager, ListAverager, count_acc, compute_confidence_interval, calc_auroc, euclidean_metric

from model.loss.context_loss import CML

class FSORTrainerOurs(Trainer):
    def __init__(self, args):
        self.grab_gpu = torch.zeros([1024, 1024, 1024], dtype=torch.double).cuda()
        super().__init__(args)
        self.train_loader, self.val_loader, self.test_loader = get_dataloader(args)
        del self.grab_gpu
        self.model = SFCM(args)
    ...
    def train(self):
        ...
        for epoch in range(1, args.max_epoch + 1):
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                result = self.post_process(data, label, gt_label)
    ...
    def evaluate_base(self, data_loader):
        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm(data_loader), 1):
                ...
                _, _, _, loss, acc, energy_auroc, logits_auroc, combine_auroc = self.post_process(data, label, gt_label)
    ...
    def auroc_process(self, bce_score, klogits, ulogits, label):
        args = self.args
        closed_way = args.closed_way if self.training else args.closed_eval_way

        bce_known_prob = bce_score[:closed_way * args.query]    # energy score
        bce_unknown_prob = bce_score[closed_way * args.query:]

        # 去除分类错误的已知类样本
        pred = torch.argmax(klogits, dim=1)

        """ Energy """
        known_scores = bce_known_prob.cpu().detach().numpy()
        unknown_scores = bce_unknown_prob.cpu().detach().numpy()

        known_scores2 = bce_known_prob[label == pred].cpu().detach().numpy()    # 去除分类错误的已知类样本
        energy_auroc = calc_auroc(known_scores2, unknown_scores) 

        if not args.SnaTCHer:
            """ logits """
            known_dist = -(klogits.max(1)[0])   # 直接利用相似距离计算auroc(without softmax)
        ...
        if not args.SnaTCHer:
            ...
            combine_auroc = calc_auroc(known_dist_score, unknown_dist_score)    # combine distance and energy
        else:
    ...
    def post_process(self, data, label, gt_label):
        """_summary_

        Args:
            data (_type_): _description_
            label (_type_): local label
            gt_label (_type_): global label
        ...
        e_klogits, e_ulogits = emb  # close classify
        f_klogits, f_ulogits = feat  # open classsify
        ...
        # # metirc loss
        # k_query = k_query.squeeze(1)
        # u_query = u_query.squeeze(1)

        # l_metric = self.model.metricLoss(k_query, label)

        # context loss(hybrid)
        instance_embs = f.normalize(self.model.instance_embs, p = 2, dim=1)
        loss_ctx = CML(instance_embs, gt_label, args)
        ...
        energy_auroc, logits_auroc, combine_auroc = self.auroc_process(energy_score, e_klogits, e_ulogits, label)
        total_loss = l_few + l_open + l_energy + args.lambda_ * loss_ctx
